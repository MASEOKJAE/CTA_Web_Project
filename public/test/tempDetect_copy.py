...
def temperature_detect(image_path):
    try:
        ...
        files = [file for file in os.listdir(current_directory) if file.endswith(".jpg")]
        ...
        for i in range(num_colors):
            ...
            if image is None:
                raise ValueError(f"[ERROR] Image could not be loaded. Check if the file exists: {image_path}")

            # 이미지 크기 변경

            # Crop image 
            tmp_image = image[y:y+crop_height, x:x+crop_width]
            file_result = f"result-{i}_crop.jpg"
            ...
            # Save the result 
            file_result = f"result-{i}.jpg"
            # cv2.imwrite(file_result, tmp_image)

            # intermediate results for verification
            file_result = f"result-{i}_colext.jpg"
            # cv2.imwrite(file_result, color_extracted)
            file_result = f"result-{i}_gray.jpg"
            # cv2.imwrite(file_result, gray_adaptive)
            file_result = f"result-{i}_edge.jpg"
